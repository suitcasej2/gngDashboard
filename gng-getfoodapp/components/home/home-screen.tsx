      ...
      {harvest ? (
        <div className="space-y-4 lg:max-w-md">
          ...
          {previewMode ? (
            <div
              className={cn(
                "flex h-16 items-center justify-between rounded-full px-6 text-white shadow-lg lg:h-[4.25rem] lg:px-8",
                "bg-[var(--brand-green)] opacity-80"
              )}
              aria-disabled
            >
              <span className="text-base font-medium lg:text-lg">
                RSVP to this harvest
              </span>
          ...
          {!previewMode ? (
            ...
            <div className="rounded-2xl border border-dashed bg-muted/30 px-4 py-5 text-sm text-muted-foreground">
              Community chat appears here after publish.
            </div>
          )}
        </div>
      ) : (
...
export function HomeScreen({
  ...
  const pickupLabel = harvest
    ...
    : null;
  const heroSrc = harvest?.headerImageUrl ?? "/HeaderImage.jpg";
      ...
      <section className="relative min-h-[58dvh] shrink-0 overflow-hidden lg:sticky lg:top-0 lg:min-h-dvh lg:flex-1">
        ...
        <div className="relative flex h-full min-h-[58dvh] flex-col px-4 pb-8 pt-[max(env(safe-area-inset-top),1rem)] lg:min-h-dvh lg:px-10 lg:pb-12 lg:pt-8 xl:px-14">
          <div className="flex items-start justify-between">
            {previewMode ? (
              <span className="rounded-full bg-[#FFF904]/95 px-3 py-1.5 text-xs font-semibold text-[var(--brand-brown)] shadow-sm">
                Draft preview
              </span>
            ) : (
